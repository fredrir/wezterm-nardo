...
local version = require "nardo.version"
...
function M.spawn_args(cfg, domain, host)
  ...
  if M.is_local(domain, host) then
    ...
    return { "sh", M.root .. "/bin/bootstrap.sh", id.name, id.prefix }
  end
  local script = bootstrap_script()
  if not script then
    return nil
  end
  -- `sh -c script name prefix` would bind name to $0; the extra word keeps it at $1.
  return { "sh", "-c", script, "sh", id.name, id.prefix }
end

---Prefixed vars are the bootstrap's contract; WEZPLUG_* are read by the binary itself.
function M.env(cfg, domain, host)
  local env = {
    WEZPLUG_USERVAR = cfg.backend.uservar,
    [id.prefix .. "_REPO"] = cfg.backend.repo,
    [id.prefix .. "_VERSION"] = cfg.backend.version or version,
  }
  if cfg.backend.log then
    env.WEZPLUG_LOG = cfg.backend.log
  end
  if M.is_local(domain, host) then
    env[id.prefix .. "_TARGET"] = platform.triple
    env[id.prefix .. "_SRC"] = M.root .. "/../backend"
    env[id.prefix .. "_BUILD"] = cfg.backend.build and "1" or "0"
    env[id.prefix .. "_BIN"] = M.resolve_path(cfg, domain, host)
  else
    env[id.prefix .. "_BUILD"] = "0"
  end
  return env
end
