...
The backend is a WezTerm pane running `wez-nardo` (the Rust process). The Lua
...
- **backend → Lua**: events, sent as WezTerm user vars
  (`ESC ] 1337 ; SetUserVar=nardo=<base64(json)> BEL`), received by the plugin
...
2. Enable mouse reporting: `?1000h ?1002h ?1003h ?1006h`, and focus reporting
   `?1004h`.
3. Emit user var `nardo_role` = `backend` (plain, base64 of the literal string).
...
| `WEZPLUG_LOG`     | append debug log lines to this file (default: no logging; 0600, symlinks refused, key names redacted). Never log to stderr. |
