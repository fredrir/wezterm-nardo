...
use crate::terminal;
use crate::uservar::{Vars, set_user_var};
...
impl<W: Write> App<W> {
    ...
    pub fn emit(&mut self, event: &Event) -> io::Result<()> {
        ...
        let osc = set_user_var(&self.vars.base, &json);
        self.write(osc.as_bytes())
    }
    ...
    fn run(&mut self, cmd: Command) -> io::Result<bool> {
        match cmd {
            ...
            Command::Auth { token } => {
                let osc = set_user_var(&self.vars.token, &token);
                self.write(osc.as_bytes())?
            }
...
mod tests {
    ...
    fn ping_answers_with_pong_user_var() {
        ...
        assert_eq!(
            a.out,
            set_user_var("demo", r#"{"t":"pong","n":7}"#).as_bytes()
        );
    }

    #[test]
    fn auth_echoes_token_on_the_derived_var() {
        let mut a = app();
        a.handle(Input::Command(Command::Auth {
            token: "abc".into(),
        }))
        .unwrap();
        assert_eq!(a.out, set_user_var("demo_token", "abc").as_bytes());
    }
    ...
    fn resize_emits_only_on_change() {
        ...
        assert_eq!(
            a.out,
            set_user_var("demo", r#"{"t":"resize","cols":30,"rows":24}"#).as_bytes()
        );
    }
    ...
    fn key_events_are_emitted() {
        ...
        assert_eq!(
            a.out,
            set_user_var("demo", r#"{"t":"key","key":"x"}"#).as_bytes()
        );
    }
}
