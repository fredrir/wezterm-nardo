...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    -h | --help) say "usage: just deploy [--from-dev|--from-prd|--from-release]"; exit 0 ;;
    *) die "unknown flag: $1" ;;
  esac
  shift
done
...
case "$mode" in
  dev)
    if ! has_backend; then
      ok "no backend crate; the plugin is pure Lua and already live from $dim$root/plugin$off"
      exit 0
    fi
    ...
    say "${dim}your config already points here (backend.path)${off}"
    ;;

  prd)
    if has_backend && [ "$triple" = unknown ]; then die "unsupported host: $(uname -sm)"; fi
    ...
    say "  ${dim}local $ns = wezterm.plugin.require \"$url\"${off}"
    if has_backend; then restart_backends "$cached"; fi
    ;;

  release)
    has_backend || die "no backend crate; --from-release only installs published binaries"
    if [ "$triple" = unknown ]; then die "unsupported host: $(uname -sm)"; fi
    command -v curl >/dev/null 2>&1 || die "curl not found"
    base="$url/releases/download/v$version"
    tmp=$(mktemp -d "/tmp/$ns-rel.XXXXXX")
    trap 'rm -rf "$tmp"' EXIT
    ...
    if [ -z "$expected" ] || [ "$expected" != "$actual" ]; then die "checksum mismatch"; fi
