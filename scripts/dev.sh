...
if [ -z "$profile" ]; then
  if [ "$live" = 1 ]; then profile=release; else profile=debug; fi
fi
bin=$(bin_for "$profile")

if [ "$mode" = check ]; then
  cd "$root/plugin"
  if lua tests/run.lua >"/tmp/$ns-dev-test.log" 2>&1; then
    ok "lua $(tail -1 "/tmp/$ns-dev-test.log")"
  else
    printf '%slua tests failed%s\n' "$red" "$off"
    tail -15 "/tmp/$ns-dev-test.log"
  fi
  if command -v luacheck >/dev/null 2>&1; then luacheck -q init.lua "$ns" || true; fi
  exit 0
fi

if [ "$mode" = once ]; then
  build "$profile" || exit 0
  restart_backends "$bin"
  exit 0
fi
...
cleanup() {
  ...
  pkill -f "class $ns-dev" 2>/dev/null
...
if [ "$live" = 0 ]; then
  sandbox_home=$(mktemp -d "/tmp/$ns-dev.XXXXXX")
  mkdir -p "$sandbox_home/.local/share/wezterm"
  say "${dim}sandbox HOME=$sandbox_home${off}"
  log="$sandbox_home/wezterm.log"
  # WEZPLUG_* are the sandbox's own contract; the plugin reads the prefixed names.
  HOME="$sandbox_home" WEZPLUG_ROOT="$root" WEZPLUG_BIN="$bin" WEZTERM_LOG=info \
    wezterm --config-file "$root/scripts/dev-config.lua" \
    start --always-new-process --class "$ns-dev" >"$log" 2>&1 &
  pids="$pids $!"
  ( tail -n +1 -f "$log" | grep --line-buffered -E "$ns:|WARN|ERROR" ) &
  pids="$pids $!"
else
  say "${dim}live: hot-swapping backends in your running WezTerm${off}"
fi
...
if has_backend; then
  ...
  ok "watching plugin (tests) — ^C to stop"
fi
wait
